...
Skip until line to just upload the data 
...
#this is a list of the classification of ASVs from the rarefied ps based on RF
...
# Step 1: Combine your phyloseq list and RF importance list into a named list of pairs
ps_rf_list <- list(
...
# Function to filter phyloseq object to important ASVs and extract combined data
...
```{r}
#Now let's make it into a df so we can plot it 
# First, pivot each combined df to long format
important_long_list <- lapply(names(important_data_list), function(name) {
  ...
  abundance_df <- combined_df %>%
    select(ASV, starts_with("Sample") | matches("^\\d")) # assuming sample columns have names like "Sample_1" or "11_Soil"
  
  taxonomy_df <- combined_df %>%
    select(ASV, all_of(taxonomy_columns))
...
final_long_df <- final_long_df %>%
  select(Sub_community, ASV, sample, abundance, everything())

# Preview
head(final_long_df)

# Extract metadata for all phyloseq objects
sample_metadata_list <- lapply(ps_list_rarefied, function(ps) {
  as(sample_data(ps), "data.frame") %>%
    rownames_to_column(var = "sample") %>%
    select(sample, MeJA_treatment, Plant_species, Sample_type)
})

# Combine all metadata into one big dataframe
...
final_long_df_with_meta  <- final_long_df_with_meta  %>%
  select(Sub_community, Plant_species, Sample_type, MeJA_treatment, sample, ASV, abundance, everything())

# Save to file
write.csv(final_long_df_with_meta, "./R output/07_random_forest/full_important_data_long.csv", row.names = FALSE)


#plot it
ggplot(final_long_df_with_meta, aes(x = ASV, y = abundance, fill = MeJA_treatment)) +
  geom_boxplot(aes(x = ASV, y = abundance, fill = MeJA_treatment))+
  geom_jitter(aes(x = ASV, y = abundance, color = MeJA_treatment), 
...
Now plot it with family information
```{r}

#make name of ASVs with Genus/Family information
df <- final_long_df_with_meta %>%
  mutate(
    Genus_display = ifelse(is.na(Genus) | Genus == "NA" | Genus == "", "Unknown", Genus),
    Genus_display = gsub("^g__", "", Genus_display),
    ...
    Family_display = gsub("^f__", "", Family_display),
...
# Plot
ggplot(df, aes(x = Family_display_short, y = abundance, fill = MeJA_treatment)) +
  geom_boxplot(aes(x = Family_display_short, y = abundance, fill = MeJA_treatment),
               outlier.shape = 8, outlier.size = 1.2)+
  geom_jitter(aes(x = Family_display_short, y = abundance, fill = MeJA_treatment), 
              position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.75), 
              shape=21, stroke= 0.5, size = 0.7, alpha = 0.7) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_fill_manual(values = color_treatments) +
  scale_color_manual(values = color_treatments) +
  labs(x = "ASV", y = "Abundance", fill = "MeJA Treatment")+
...
ggplot(full_mean_importance ,
       aes(x = Family, y = Mean_importance)) +
  geom_point(size = 4, alpha = 0.6) +
  labs(y = "Mean importance") +
  #scale_color_manual(values = color_JAtreatments) +
  #geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  facet_grid(Compartment + Species ~ Order, scales = "free_x", space = "free_x") +
...
ggplot(asv_counts, aes(x = Family, y = Number_of_ASVs, fill = Compartment)) +
  ...
  facet_grid(Species ~ Order, scales = "free_x", space = "free_x") +
