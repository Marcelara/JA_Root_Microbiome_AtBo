...
# Function to filter phyloseq object to important ASVs and extract combined data with relative abundances
...
#make Burkholderia name shorter, rename df because its too long name 
...
#make new labels with Genus and ASV number
df <- df  %>%
  mutate(
...
#let's order it by Phylum-Order-Family
...
#summary df with ASV counts
df_summary <- full_mean_importance %>%
  group_by(Species, Compartment) %>% 
  count(Family, Order, Phylum, sort = TRUE)

#Add category for those orders that we want to highlight
