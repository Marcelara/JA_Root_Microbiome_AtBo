...
#save df
BO_asv_list.endo <- lapply(ps_list_neutral_BO_Root, function(x) {
  taxa_names(subset_taxa(x, fit_class == "Above prediction"))
})

saveRDS(BO_asv_list.endo , "./R output/04b_NM_Endo/BO_asv_list.rds")
...
#save df
AT_asv_list.endo <- lapply(ps_list_neutral_AT_Root, function(x) {
  taxa_names(subset_taxa(x, fit_class == "Above prediction"))
})

saveRDS(AT_asv_list.endo, "./R output/04b_NM_Endo/AT_asv_list.rds")
...
ggsave(export_venn,
  height = 5, width = 10,
  file = "./Plots/04b_Neutral_models_rhizosphere/NeutralModel_Veen_BO_AT_endosphere.svg"
)
```
...
First save the output in rds objects 
```{r}

predictions_listed_AT_Root #df with all ASVs and their classification
AT_Root_predicted_list #ps classified as above-neutral and below
neutral_list_AT_Root

#make a list 
NM_AT_Root <- list(predictions_listed_AT_Root =predictions_listed_AT_Root,
                   AT_Root_predicted_list = AT_Root_predicted_list,
                   neutral_list_AT_Root=neutral_list_AT_Root)

#save it 
saveRDS(NM_AT_Root, "./R output/04b_NM_Endo/NM_AT_Root.rds")


predictions_listed_BO_Root
BO_Root_predicted_list
neutral_list_BO_Root

#make a list 
NM_BO_Root <- list(predictions_listed_BO_Root =predictions_listed_BO_Root,
                   BO_Root_predicted_list = BO_Root_predicted_list,
                   neutral_list_BO_Root = neutral_list_BO_Root)

#save it 
saveRDS(NM_BO_Root, "./R output/04b_NM_Endo/NM_BO_Root.rds")



predictions_listed_AT_soil
AT_soil_predicted_list
neutral_list_AT_soil

#make a list 
NM_AT_Rhizo <- list(predictions_listed_AT_soil =predictions_listed_AT_soil,
                   AT_soil_predicted_list = AT_soil_predicted_list,
                   neutral_list_AT_soil = neutral_list_AT_soil)

#save it 
saveRDS(NM_AT_Rhizo, "./R output/04a_NM_Rhizo/NM_AT_Rhizo.rds")

predictions_listed_BO_soil
BO_soil_predicted_list
neutral_list_BO_soil

#make a list 
NM_BO_Rhizo <- list(predictions_listed_BO_soil =predictions_listed_BO_soil,
                   BO_soil_predicted_list = BO_soil_predicted_list,
                   neutral_list_BO_soil = neutral_list_BO_soil)

#save it 
saveRDS(NM_BO_Rhizo, "./R output/04a_NM_Rhizo/NM_BO_Rhizo.rds")

#save space
rm(NM_AT_Rhizo, NM_AT_Root, NM_BO_Rhizo, NM_BO_Root)
...
save.image("./R output/env.4_neutral_models_rhizo_endo.RData")
...
# 4b.10 plot a heat tree of above_expected asvs[SKIP]
