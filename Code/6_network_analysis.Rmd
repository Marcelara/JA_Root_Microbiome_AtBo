...
```{r}
#Now, let's add the classification as module hub or module connector 

#  Function to classify hubs and connectors
...
}) #ok, perfect, now it matches the `network metrics` summary


#get info of only those classified as central ASVs (hubs+connectors)
...
}) #this is the one Pedro used for the Fisher test
...
#clean-up for space
rm(add_module_roles, Nodes_list, central_asv_list, central_keystone_asv_list, Nodes_AT_root, Nodes_AT_soil,
   Nodes_BO_root, Nodes_BO_soil)

```

```{r}

x <- central_keystone_asv_df %>% filter(keystone_taxa == "Keystone")
...
Quick exploration with plots (skip)
```{r}

library(tidyverse)

# Clasificar los nodos de AT_root como keystone, hub o connector
Nodes_AT_root <- Nodes_AT_root %>%
    mutate(module_hub = ifelse(Zi > 2.5, "Module Hub", "Non-Hub"),
           module_connector = ifelse(Pi > 0.62, "Module Connector", "Non-Connector"),
           is_keystone = keystone_taxa == "Keystone" | 
                         module_hub == "Module Hub" | 
                         module_connector == "Module Connector")

# Quedarse solo con edges donde ambos nodos son keystones/hubs/connectors

Nodes_AT_root <- Nodes_AT_root %>%
    mutate(Family = ifelse(grepl("^f__", Family), gsub("^f__", "", Family), Family))


edges_with_taxa <- Edges_AT_root %>%
    left_join(Nodes_AT_root %>% select(Row.names, Family, is_keystone), 
              by = c("from" = "Row.names")) %>%
    left_join(Nodes_AT_root %>% select(Row.names, Family, is_keystone), 
              by = c("to" = "Row.names")) %>%
    dplyr::rename(from_family = Family.x, from_keystone = is_keystone.x,
           to_family = Family.y, to_keystone = is_keystone.y) %>%
    filter(from_keystone | to_keystone)
  
head(edges_with_taxa)

edges_with_taxa <- Edges_AT_root %>%
    left_join(Nodes_AT_root %>% select(Row.names, Family, module_hub, module_connector), 
              by = c("from" = "Row.names")) %>%
    left_join(Nodes_AT_root %>% select(Row.names, Family, module_hub, module_connector), 
              by = c("to" = "Row.names")) %>%
    dplyr::rename(from_family = Family.x, from_hub = module_hub.x, from_connector = module_connector.x,
                  to_family = Family.y, to_hub = module_hub.y, to_connector = module_connector.y) %>%
    filter(from_hub == "Module Hub" | from_connector == "Module Connector" |
           to_hub == "Module Hub" | to_connector == "Module Connector")

 
library(ggalluvial)

edges_with_taxa %>%
    count(from_family, to_family, positive_negative) %>%
    ggplot(aes(axis1 = from_family, axis2 = to_family, y = n, fill = positive_negative)) +
    geom_alluvium(width = 0.2, alpha = 0.8) +
    geom_stratum() +
    geom_text(stat = "stratum", aes(label = after_stat(stratum)), size = 3) +
    scale_fill_manual(values = c("positive" = "#66c2a5", "negative" = "#fc8d62")) +
    labs(x = "Family", y = "Number of edges", fill = "Edge Type",
         title = "AT Root - Family connections (Hubs + Connectors)") +
    theme_minimal() +
...
# Clasificación (Keystone/Hubs/Connectors)
all_nodes <- all_nodes %>%
    mutate(module_hub = ifelse(Zi > 2.5, "Module Hub", "Non-Hub"),
           module_connector = ifelse(Pi > 0.62, "Module Connector", "Non-Connector"),
...
#important families we've been highlighting 
main_families <- c(
  "Burkholderiaceae", 
  "Chitinophagaceae",
  "Comamonadaceae",
...
#make new column with these families written and otherwise, 'Other' will appear
all_nodes  <- all_nodes  %>%
...
#lets check to which order they belong to
...
#let's just write them here so we can view them
...
#color vectors 
# Order colors - you can adapt these if needed
Order_col <- c(
...
#Full; also with regular nodes 

# Define keystone shapes and regular shape for Other
keystone_shapes <- c(
    "Keystone (manual)" = 23,
    "Hub" = 21,
    "Connector" = 22,
...
Highlighting Mucilaginibacter
```{r}
#Now if you want to highlight Mucilinagibacter;
...
#let's load the list we already have with the JA-responsive ASVs per method
important_ASV_list <- readRDS("./R output/10_summary_results/important_ASV_list.rds")

#keep only those of random forest
na_asv_list <- important_ASV_list$NA_ASV_list

#how many DA ASVs were per plant species and root compartment?
sapply(na_asv_list, length)

#load nVennR to make ven diagram
library(nVennR)

# Draw the Venn diagram
venn <- plotVenn(na_asv_list, systemShow = TRUE) #this will give you an .svg file that you can save 
