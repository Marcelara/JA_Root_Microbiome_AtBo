...
ggplot(all_nodes, aes(x = Zi, y = Pi,
    ...
    geom_point(data = all_nodes %>% filter(Keystone_Type == "Other"),  # Faded regular nodes (no outline)
               aes(color = Family_simplified), 
               size = 2, alpha = 0.5) + 
    geom_point(data = all_nodes %>% filter(Keystone_Type != "Other"), # Important-Keystone with outline
...
Cambiar supplementary figures (if needed)

Checar puntos de la discusion
