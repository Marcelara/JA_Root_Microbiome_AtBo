...
If you come from script 4b, continue here but first clean up a little. If starting from scratch, load data in line 78
```{r}
...
Load data if starting from scratch
```{r}

#load ps_root_soil phyloseq object. [save it later from Beta diversity script, go to line 121 here]
...
#Make Label_full --> pimp the names for the heatmaps, first family then genus
...
#re-order Taxonomy so they appear in order of "importance" Phylum-Order-Family
library(forcats)
library(dplyr)
...
# Order Order within each Phylum by total ASVs
...
# Order Family within each Order by total ASVs
...
#make vector for colors (change because not all of them are there)
...
# Extraer las familias presentes en row_anno_rhizo
families_to_keep <- unique(row_anno_rhizo$Family_simplified)

# Filtrar el vector de colores
...
# Create a unique group ID combining Species and Compartment
combined_da <- combined_da %>%
...
# Create a list of ASVs for each group
DA_ASVs <- combined_da %>%
  group_by(Group) %>%
...
venn <- plotVenn(DA_ASVs, systemShow = TRUE) #this will give you an .svg file that you can save 

```


#Direction Log2FC summary
```{r}
...
combined_da_df <- combined_da%>%
  ...
  dplyr::select(-(DADA2_ID:Label_full))%>% 
...
# Create stacked bar plot with percentage labels inside the bars
...
```{r}
#other summaries;
#By Plant species and treatment
summary_table_Sp_JA <- combined_da_df %>%
  dplyr::group_by(Compartment, Treatment) %>%
  summarise(n_ASVs = n_distinct(ASV),
            n_positive_LFC = sum(log2FoldChange > 0),
            n_negative_LFC = sum(log2FoldChange < 0),
            .groups = "drop")

#let's summarize taxonomy
taxonomy_summary <- combined_da_df %>%
  mutate(Direction = ifelse(log2FoldChange > 0, "Positive", "Negative")) %>%
  group_by(Plant_Species, Compartment, Phylum, Class, Order, Family) %>%
  summarise(n_ASVs = n_distinct(ASV), .groups = "drop")

taxonomy_summary_Treatment <- combined_da_df %>%
  mutate(Direction = ifelse(log2FoldChange > 0, "Positive", "Negative")) %>%
  group_by(Plant_Species, Compartment, Treatment, Phylum, Class, Order, Family) %>%
  summarise(n_ASVs = n_distinct(ASV), .groups = "drop")

write.csv(taxonomy_summary, "./R output/05_differential_abundance/taxonomy_summary_SpRc.csv")

```

Explore by Plant Species 
```{r}

At_combined <- combined_da_df %>% subset(Plant_Species == "A. thaliana") %>% droplevels() 
unique(At_combined$ASV)
unique(At_combined$Family)

#Only Endo
At_Endo <- At_combined %>% 
               subset(Compartment == "Root") %>% 
               group_by(Family) %>% 
               summarise(n_ASVs = n_distinct(ASV), .groups = "drop")%>%
               arrange(n_ASVs)

#Group by Family
At_family <- At_combined %>% 
  mutate(Direction = ifelse(log2FoldChange > 0, "Positive", "Negative")) %>%
  group_by(Compartment, Family) %>%
  summarise(n_ASVs = n_distinct(ASV), .groups = "drop")%>%
  arrange(n_ASVs)

unique(At_family$Family)

#B. ole
Bo_combined <- combined_da_df %>% subset(Plant_Species == "B. oleracea") %>% droplevels()
unique(Bo_combined$ASV)
unique(Bo_combined$Family)
...
#subset
At <- taxonomy_summary_Treatment  %>% subset(Plant_Species == "A. thaliana") %>% droplevels()
Bo <- taxonomy_summary_Treatment %>% subset(Plant_Species == "B. oleracea") %>% droplevels()

write.csv(At, "./R output/05_differential_abundance/taxonomy_summary_At.csv")
write.csv(Bo, "./R output/05_differential_abundance/taxonomy_summary_Bo.csv")


#subset
Endo  <- taxonomy_summary_Treatment  %>% subset(Compartment == "Root") %>% droplevels()
Rhizo <- taxonomy_summary_Treatment %>% subset(Compartment == "Soil") %>% droplevels()

write.csv(Endo, "./R output/05_differential_abundance/taxonomy_summary_Endo.csv")
write.csv(Rhizo , "./R output/05_differential_abundance/taxonomy_summary_Rhizo.csv")
