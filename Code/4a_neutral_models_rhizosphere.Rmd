...
Skip if you started in the previous scripts 
```{r}
...
```{r}
#load("./R output/env.3_alpha_diversity.RData")

#let's clean what we dont need
rm(alpha_diversity, aov_list, aovTukey, aovTukey.e, aovTukey.r, df, endo, rhizo, merg_to_ps,
   p_alpha_4, diversity_list, ps_list_CSS2)
...
names(MeJA_treatment_pal) <- c("Control", "0.1 mM MeJA", "1 mM MeJA", "Caterpillar OS")
...
treatment_names <- c(
  ...
  "MeJA_1.0" = "1 mM \n MeJA", "oral_secretion" = "Caterpillar \n OS"
)
```
...
# 4a.6 one model for each Bole-Rhizosphere treatment, using all BO_soil samples as a pool
...
ggsave(Neutral_plots_list_BO_soil_g,
  height = 9, width = 12,
  file = "./Plots/04a_Neutral_models_rhizosphere/NeutralModels_BO.svg"
)
...
# make a list of tibles, including percentage of each fit class [Supplementary Table SX]
...
#### 4a.6.4 Add neutral fit classes of ASVs as metadata to your original phyloseq objects
...
sample_data_df <- as(phyloseq::sample_data(BO_soil_predicted_list$Brassica_oleraceae.Soil.above), "data.frame")
...
##Get a vector with the ASV names 
BO_asv_list <- lapply(ps_list_neutral_BO_soil, function(x) {
  taxa_names(subset_taxa(x, fit_class == "Above prediction"))})

#let's save this;
saveRDS(BO_asv_list, "./R output/04a_NM_Rhizo/BO_asv_list.rds")

```

[MA]In here I would be interested in those that are above-neutral in JA-inducing treatments and not in controls


# 4a.7 one model for each Atha-Rhizo, using all AT_soil samples as a pool
...
#### 4a.7.6 check stats of treatment effects on each subcommunity[SKIP]
...
#save df
AT_asv_list <- lapply(ps_list_neutral_AT_soil, function(x) {
  taxa_names(subset_taxa(x, fit_class == "Above prediction"))
})

saveRDS(AT_asv_list, "./R output/04a_NM_Rhizo/AT_asv_list.rds")
...
ggsave(export_venn,
  height = 5, width = 10,
  file = "./Plots/04a_Neutral_models_rhizosphere/NeutralModel_Veen_BO_AT.svg")
```
